...
class DbWrapper:
    def __init__(self):
        self.client = MongoClient(
            'mongodb+srv://York:kwlwhTCkMkAjTBXq@cluster0.mrkq1.mongodb.net/air?retryWrites=true&w=majority')
    ...
    def get_data(self, limit=None, minute=False):
        if minute:
            ...
            if limit:
                cursor = self.db.air60.find().sort([('_id', -1)]).limit(limit)
            else:
                cursor = self.db.air60.find().sort([('_id', -1)])
